...
int tcp_receive(tcpsock_t *socket, void *buffer, int *buf_size) {
    ...
    timeout.tv_usec = 0;
    setsockopt(socket->sd, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout, sizeof(timeout));
    *buf_size = recv(socket->sd, buffer, *buf_size, 0);
    TCP_DEBUG_PRINTF(*buf_size == -1, "Recv() : connection to peer timed out\n");
    TCP_ERR_HANDLER(*buf_size == -1, close(socket->sd));
    TCP_ERR_HANDLER(*buf_size == -1, return TCP_CONNECTION_CLOSED);
